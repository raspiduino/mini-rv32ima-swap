...
// Just default RAM amount is 12MB.
...
static uint32_t store4(uint32_t ofs, uint32_t val) { 
    //printf("%08lx %08x\n", ofs, val);
    //*(uint32_t*)(ram_image + ofs) = val;*/
    fseek(f, ofs, SEEK_SET);

    uint32_t r = val;
    fwrite(((uint8_t *)&r), 1, 1, f);
    fwrite(((uint8_t *)&r + 1), 1, 1, f);
    fwrite(((uint8_t *)&r + 2), 1, 1, f);
    fwrite(((uint8_t *)&r + 3), 1, 1, f);
}

static uint16_t store2(uint32_t ofs, uint16_t val) {
    //printf("%08lx %08x\n", ofs, val);
    /*(uint16_t*)(ram_image + ofs) = val;*/
    fseek(f, ofs, SEEK_SET);
...
static uint32_t load4(uint32_t ofs) {
    //printf("%08lx\n", ofs);
    //return *(uint32_t*)(ram_image + ofs);
    fseek(f, ofs, SEEK_SET);

    uint32_t result;
    fread(((uint8_t *)&result), 1, 1, f);
    fread(((uint8_t *)&result + 1), 1, 1, f);
    fread(((uint8_t *)&result + 2), 1, 1, f);
    fread(((uint8_t *)&result + 3), 1, 1, f);
    return result;
}

static uint16_t load2(uint32_t ofs) {
    //printf("%08lx\n", ofs);
    //return *(uint16_t*)(ram_image + ofs);
    fseek(f, ofs, SEEK_SET);
